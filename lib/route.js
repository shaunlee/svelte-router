...
export function link(node, attrs = {}) {
  ...
  const unsubscribe = currentHref.subscribe(state => {
    ...
    node.classList.toggle(attrs.active, l !== (config.mode !== 'hash' ? config.base : '') + '/' && r.startsWith(l))
    node.classList.toggle(attrs.exactActive, l === r)
  })
...
export function urlQuery(path, query = {}) {
