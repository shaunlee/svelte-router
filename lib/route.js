...
router.push = (path, query = {}, opts = { overwrite: true }) => {
  ...
  history.pushState({}, '', urlQuery(path, query, opts))
  updateState()
}

router.replace = (path, query = {}, opts = { overwrite: true }) => {
  path = config.base + (config.mode === 'hash' ? '#' : '') + path
  history.replaceState({}, '', urlQuery(path, query, opts))
  updateState()
}
...
function buildPatterns() {
  ...
  for (const route of config.routes) {
    if (route.path === '*') {
      ...
      route.pattern = pattern.indexOf('(?') > -1
        ? new RegExp(`^${pattern}$`)
        : pattern
    }
  }
...
export function updateState() {
  ...
  for (const route of config.routes) {
    if (route.pattern instanceof RegExp) {
      ...
      router.set({ path, component: route.component, params: {}, query })
      break
    }
  }
