...
```html
<script>
  import { router } from '@shaun/svelterouter'
</script>
...
Therefore, if you are already familiar with [Browser History APIs](https://developer.mozilla.org/en-US/docs/Web/API/History_API), manipulating history will feel familiar when using `@shaun/svelterouter`.

It is worth mentioning that `@shaun/svelterouter` navigation methods (push, replace, go) work consistently no matter the kind of `mode` option is passed when creating the router instance.
...
```javascript
import { createRouter } from '@shaun/svelterouter'
